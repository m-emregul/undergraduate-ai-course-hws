...
X = 102             #labirentin boyutları
Y = 102
K = 100             #engel sayısı = K*4
...
def generate_obstacle(Maze, l, w, K):       #K adet L şeklinde engel rastgele yerlere yerleştirilir
    ...
    count = 0
    while count < num:
        rx = random.randint(2, l - 4)           #labirent üzerinde rastgele bir nokta seçilir. O noktanın olduğu bölgeye, eğer uygunsa L şeklinde engel çizilir.
        ry = random.randint(2, w - 4)
        if isWalkable(Maze,[rx,ry]) and isWalkable(Maze,[rx,ry+1]) and isWalkable(Maze,[rx+1,ry]) and isWalkable(Maze,[rx+2,ry]) and isWalkable(Maze,[rx+3,ry]):
            Maze[rx][ry] = 5
            Maze[rx][ry + 1] = 5
            Maze[rx + 1][ry] = 5
            Maze[rx + 2][ry] = 5
    ...
    while count < num:
        rx = random.randint(2, l - 4)
        ry = random.randint(2, w - 4)
        if isWalkable(Maze,[rx,ry]) and isWalkable(Maze,[rx-1,ry]) and isWalkable(Maze,[rx,ry+1]) and isWalkable(Maze,[rx,ry+2]) and isWalkable(Maze,[rx,ry+3]):
            Maze[rx][ry] = 5
            Maze[rx-1][ry] = 5
            Maze[rx][ry+1] = 5
            Maze[rx][ry+2] = 5
...
def fitness(Maze, l, w, arr, generation):  #degerlendirme fonksiyonu.  l ve w labirentin boyuntları.    arr popülasyondaki bir eleman.  generation, jenerasyon sayısı
    ...
    for n in arr:
        ...
        if point[0] == l-2 and point[1] == w-2:
            draw(Maze, l, w, arr)                                       #sonuç bulunduysa yol labirent üzerinde çizilir.
            display_maze(Maze, l, w)                                    #labirent ekrana bastırılır
            print("Target is found ! ")
            print("the path : {}" .format(arr[:fvalue]))                         #ilgili bilgiler ekrana basılır ve program durdurulur.
...
def display_maze(Maze, l, w):                   #Labirenti i pygame kütüphanesini kullanarak ekrana bastırır
    ekran = pygame.display.set_mode((660, 600))
    RED = (255, 0, 0)
    GREEN = (0, 255, 0)
    BLUE = (0, 0, 255)
    BLACK = (0, 0, 0)
    ...
    for y in range(X):
        for x in range(Y):
            ...
            if Maze[x][y] == 5:
                ...
                pygame.draw.rect(ekran, BLUE, rect)
            elif Maze[x][y] == 0:
